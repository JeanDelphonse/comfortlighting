...
## Project Overview
Flask web application for ComfortLighting.net. A lead inventory and sales pipeline management system with AI-powered proposal and contract generation, lead user assignment, and sales activity/expense tracking. Deployed on GoDaddy cPanel via Passenger WSGI.
...
├── docs/
│   ├── ComfortLighting_PRD.docx
│   ├── ComfortLighting_Proposal_Workflow_PRD_Python.docx
│   ├── ComfortLighting_Contract_Workflow_PRD.docx
│   ├── ComfortLighting_LeadUserAssignment_PRD.docx
│   ├── ComfortLighting_SalesActivity_Expenses_PRD.docx
│   └── SalesStatusBlank.xlsx
└── logs/
...
3. `migration_system_config.sql` — create system_config, seed IRS mileage rate
4. `migration_create_lead_activities.sql` — create expense_categories + lead_activities, seed categories
...
### CSRF on AJAX
All AJAX POST requests must include the header `X-CSRFToken`. Config has `WTF_CSRF_HEADERS = ['X-CSRFToken']`. Pass token via JS variable set from `{{ csrf_token() }}`.
...
include = ['passenger_wsgi.py','app.py','config.py','requirements.txt','.htaccess',
           ...
           'sql/migration_system_config.sql','sql/migration_create_lead_activities.sql']
app_files = []
...
- Visible to assigned user and Admin only (403 for other roles)
...
- Passenger strips the `/comfortlighting/` URL prefix before passing to Flask — this is why all JS URLs must use `url_for`
